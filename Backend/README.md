...
این پروژه Backend اجرایی سامانه آموزشی تک‌مدرسه‌ای هم‌آموز برای مدرسه بعثت است و از سبک **Modular Monolith** استفاده می‌کند: مرز دامنه‌ها مستقل است، اما API، تراکنش‌ها و استقرار در یک سرویس Django باقی می‌مانند.

## قابلیت‌های پیاده‌سازی‌شده

- مجموعه واحد، مدرسه ثابت بعثت، سال تحصیلی، نوبت، پایه و کلاس با ظرفیت
- Custom User، JWT چرخشی، blacklist خروج و نقش‌های سامانه
- جداسازی داده در سطح مجموعه، مدرسه، کلاس و درس دبیر
...
- ورود مستقیم و idempotent پوشهٔ `Data/Excel` و `Data/Photo`؛ ۱۲ فایل جامع قبل از سه کارنامهٔ مستقل آزمون درسی پردازش می‌شوند و ردیف خام/فرمول برای ممیزی نگه داشته می‌شود
...
برای ورود مستقیم دادهٔ موجود در مخزن (بدون ساختن `ImportJob`) اجرا کنید:

```bash
python manage.py ingest_data_directory
```

این فرمان همهٔ فایل‌های `Data/Excel` را ابتدا inventory می‌کند، دادهٔ معتبر ۱۲ فایل جامع را در جداول رسمی می‌نویسد، سپس هر سه فایل `کارنامه تفصیلی آزمون تابستانه ...` را در مدل/endpoint مستقل آزمون درسی ثبت می‌کند و در پایان عکس‌های `Data/Photo` را با کد ملی تطبیق می‌دهد. ردیف‌های نامعتبر حذف نمی‌شوند؛ در `DataSourceRow` یا `SubjectExamResult` با وضعیت و خطا باقی می‌مانند. اجرای دوباره بر اساس checksum و شمارهٔ ردیف idempotent است.
...
مدرسه مقصد در تمام روابط و Import/Reportها به‌صورت ثابت مدرسه بعثت resolve می‌شود و از Client قابل انتخاب نیست. برای منابع مجموعه‌ای از `X-Organization-ID` استفاده می‌شود. در خواندن، نبود هدر، Query را به حوزه مجاز کاربر محدود می‌کند. در نوشتن، کاربر غیر از `system_admin` باید Scope مجموعه صریح بفرستد. UUID نامعتبر، حوزه غیرمجاز یا تفاوت Scope هدر با شیء مقصد پاسخ 403 می‌گیرند.
