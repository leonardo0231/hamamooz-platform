...
def record_snapshot(record):
    return {
        field: _serialize_value(getattr(record, field))
        for field in RECORD_SNAPSHOT_FIELDS
    }
...
def bulk_record_attendance(*, session, items, actor, request=None):
    ...
    valid_enrollments = {
        ...
        for enrollment in active_enrollments_for_class(session).filter(
    ...
    for item in items:
        ...
        if created:
            record = AttendanceRecord(
                session=session, enrollment=enrollment, recorded_by=actor
            )
            before = None
        else:
...
def finalize_attendance_session(*, session, actor, request=None):
    ...
    _validate_session_writer(session=session, actor=actor)
    expected_ids = set(
        active_enrollments_for_class(session).values_list("id", flat=True)
    )
    recorded_ids = set(
    ...
    if missing or extra:
        raise ValidationError(
            {
                "records": {
                    "missing_enrollment_ids": [
                        str(value) for value in sorted(missing, key=str)
                    ],
                    "invalid_enrollment_ids": [
                        str(value) for value in sorted(extra, key=str)
                    ],
                }
            }
        )
...
def review_absence_excuse(*, record, approved, note, actor, request=None):
    ...
    record_audit(
        action="attendance.excuse_approved"
        if approved
        else "attendance.excuse_rejected",
        actor=actor,
        request=request,
        entity=record,
        organization_id=record.organization_id,
...
def _schedule_notification(notification):
    ...
    transaction.on_commit(
        lambda: dispatch_parent_notification.delay(str(notification.id))
    )
...
def _guardians_for_student(student):
    ...
    return [
        link.guardian
        for link in StudentGuardian.objects.filter(student=student).select_related(
            "guardian"
        )
    ]
...
def queue_record_parent_notifications(*, record, channels=None, actor=None):
    ...
    channels = channels or (policy.notification_channels if policy else ["in_app"])
    if not channels:
        channels = ["in_app"]
...
def evaluate_policy_alerts(*, policy, actor=None, request=None):
    ...
    for enrollment in enrollment_list:
        for scope in [AttendanceSession.Scope.DAILY, AttendanceSession.Scope.PERIOD]:
            ...
            matching_alert = (
                active_alerts.filter(severity=severity).first() if severity else None
            )
            for stale_alert in active_alerts.exclude(
                pk=getattr(matching_alert, "pk", None)
            ):
...
def _queue_alert_notifications(*, alert, policy, actor=None):
    ...
    channels = policy.notification_channels or ["in_app"]
