...
class AttendanceSessionViewSet(AuditedModelViewSet):
    ...
    def roster(self, request, pk=None):
        ...
        return Response(
            {"session": str(session.id), "count": len(data), "results": data}
        )
...
class AttendanceAlertViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def evaluate(self, request):
        ...
        alerts = evaluate_policy_alerts(
            policy=policy, actor=request.user, request=request
        )
    ...
    def acknowledge(self, request, pk=None):
        alert = acknowledge_alert(
            alert=self.get_object(), actor=request.user, request=request
        )
...
class AttendanceReportViewSet(viewsets.ViewSet):
    ...
    def _validate_enrollment_scope(self, request, enrollment):
        ...
        if enrollment.school_id not in set(
            school_ids
        ) or enrollment.class_section_id not in set(class_ids):
    ...
    def school(self, request):
        ...
        for class_section in class_list:
            ...
            class_total = sum(
                metrics_map[enrollment.id]["total_sessions"]
                for enrollment in class_enrollments
            )
            class_absent = sum(
                metrics_map[enrollment.id]["absence_count"]
                for enrollment in class_enrollments
            )
