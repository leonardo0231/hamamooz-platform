...
def attendance_date_range(
    *, academic_year, date_from=None, date_to=None, lookback_days=None
):
...
def enrollment_metrics_map(
    *, enrollment_ids, date_from, date_to, scope=None, include_excused=True
):
    ...
    rows = (
        ...
        .annotate(
            ...
            excused=Count(
                "id", filter=Q(status=AttendanceRecord.Status.ABSENT_EXCUSED)
            ),
            unexcused=Count(
                "id", filter=Q(status=AttendanceRecord.Status.ABSENT_UNEXCUSED)
            ),
    ...
    return {
        enrollment_id: results.get(enrollment_id, empty.copy())
        for enrollment_id in enrollment_ids
    }


def enrollment_metrics(
    *, enrollment, date_from, date_to, scope=None, include_excused=True
):
...
def active_enrollments_for_class(session):
    return Enrollment.objects.filter(
        ...
        "student", "school", "academic_year", "grade_level", "class_section"
    )
