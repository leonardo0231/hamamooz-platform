...
class AttendancePolicySerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if (
            request
            and school
            and school.id not in set(accessible_school_ids(request.user))
        ):
...
class AttendanceSessionSerializer(serializers.ModelSerializer):
    ...
    taken_by_name = serializers.CharField(
        source="taken_by.get_full_name", read_only=True
    )
        ...
        if request:
            ...
            if instance.school_id not in set(school_ids):
                raise serializers.ValidationError(
                    {"school": "به این شعبه دسترسی ندارید."}
                )
            if instance.class_section_id not in set(class_ids):
                raise serializers.ValidationError(
                    {"class_section": "به این کلاس دسترسی ندارید."}
                )
            instance.taken_by = getattr(self.instance, "taken_by", request.user)
            if not can_manage_session(request.user, instance):
                raise serializers.ValidationError(
                    "برای ایجاد یا ویرایش این جلسه دسترسی ندارید."
                )
        instance.full_clean(
            exclude=["id", "taken_by"] if not instance.taken_by_id else ["id"]
        )
        return attrs
...
class AttendanceRecordSerializer(serializers.ModelSerializer):
    ...
    student_number = serializers.CharField(
        source="enrollment.student_number", read_only=True
    )
    class_title = serializers.CharField(
        source="session.class_section.title", read_only=True
    )
...
class AttendanceAlertSerializer(serializers.ModelSerializer):
    ...
    class_title = serializers.CharField(
        source="enrollment.class_section.title", read_only=True
    )
    school_name = serializers.CharField(source="school.name", read_only=True)
...
class AttendanceReportQuerySerializer(serializers.Serializer):
    ...
    scope = serializers.ChoiceField(
        choices=AttendanceSession.Scope.choices, required=False
    )
    ...
    def validate(self, attrs):
        if (
            ...
            raise serializers.ValidationError(
                {"date_to": "پایان بازه باید بعد از شروع آن باشد."}
            )
        return attrs
