...
class ReportBatch(TimeStampedUUIDModel):
    ...
    requested_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="requested_report_batches"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["school", "academic_year", "term", "status"],
                name="reports_batch_school_term_idx",
            )
        ]
    ...
    def progress_percent(self):
        return (
            ...
            else int((self.completed_count + self.failed_count) * 100 / self.total_count)
        )
...
class ReportBatchItem(TimeStampedUUIDModel):
    ...
    enrollment = models.ForeignKey(
        "students.Enrollment", on_delete=models.PROTECT, related_name="report_batch_items"
    )
    report = models.OneToOneField(
        ReportArchive, null=True, blank=True, on_delete=models.PROTECT, related_name="batch_item"
    )
    status = models.CharField(
        max_length=20, choices=Status.choices, default=Status.QUEUED, db_index=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["batch", "enrollment"], name="uq_report_batch_enrollment"
            )
        ]
