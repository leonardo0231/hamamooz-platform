...
هسته‌ای عملیاتی برای یک مدرسه ثابت، مدرسه بعثت، که ساختار آموزشی، کاربران و نقش‌ها، دانش‌آموز، ثبت‌نام تاریخ‌مند، درس، نمره، کارنامه، حضور و غیاب، Import و عملیات استقرار را با جداسازی امن مجموعه و کلاس پوشش دهد.

## ماتریس تحویل

| نیاز MVP | وضعیت | محل پیاده‌سازی |
|---|---:|---|
| مجموعه واحد، مدرسه بعثت، سال، نوبت، پایه، کلاس و ظرفیت | کامل | `organizations` |
| نقش‌های سامانه و نقش‌های محدود به مدرسه بعثت | کامل | `accounts.RoleAssignment` |
...
| تغییر کلاس با حفظ تاریخچه | کامل | Service layer دانش‌آموز |
...
- `seed_demo` دقیقاً یک مدرسه ثابت بعثت و داده پایه را به‌شکل idempotent ایجاد می‌کند.
- Queryهای لیست صفحه‌بندی شده و Scope مدرسه بعثت/کلاس/درس قبل از Serialize اعمال می‌شود.
- دبیر فقط CourseOfferingهای خودش و کلاس‌های مرتبط را می‌بیند.
- نوشتن برای کاربر غیرسیستمی بدون `X-Organization-ID` صریح رد می‌شود.
