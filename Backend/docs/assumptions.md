...
1. The repository was empty at bootstrap, so no existing backend, migrations, dependencies, workflows or UI files existed to preserve.
...
5. MVP exposes backend APIs, admin and API documentation only; no temporary user interface is bundled.
