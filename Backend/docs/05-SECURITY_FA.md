...
|---|---|---|
| `system_admin` | کل سامانه | همه مجموعه‌ها و مدرسه بعثت |
| `organization_admin` | مجموعه | ساختار و عملیات مدرسه بعثت |
| `school_manager` | مدرسه بعثت | کاربر و عملیات مدرسه |
| `educational_deputy` | مدرسه بعثت | آموزش، review، lock و اصلاح رسمی |
| `operator` | مدرسه بعثت | ورود داده، ثبت‌نام و Import |
| `teacher` | مدرسه بعثت + درس/کلاس | ارزیابی، نمره و حضور زنگ خودش |

هر کاربر در محدوده مجموعه و مدرسه بعثت نقش مشخص دارد. مدیریت نقش سلسله‌مراتبی است؛ مدیر مدرسه نمی‌تواند مدیر سیستم یا مدیر مجموعه را مدیریت کند.

## الگوریتم Tenant isolation

1. JWT هویت کاربر فعال را تعیین می‌کند.
2. RoleAssignmentهای فعال، مجموعه‌ها و شعب مجاز را می‌سازند.
3. `X-Organization-ID` parse و با Scope مجاز intersect می‌شود؛ مدرسه همیشه بعثت است.
