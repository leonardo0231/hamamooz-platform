...
function TrendChart({ option, label, className }) {
  ...
  return chartShell({ label, className, children: html`<svg class="echart__svg echart__svg--trend" viewBox=${`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet" focusable="false" aria-hidden="true">
    <title key="trend-title">${label}</title>
    ...
    ${area && html`<path key="trend-area" d=${area} fill="rgba(15,118,110,.14)" stroke="none"/>`}
...
function RadarChart({ option, label, className }) {
  ...
  return chartShell({ label, className, children: html`<svg class="echart__svg echart__svg--radar" viewBox=${`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet" focusable="false" aria-hidden="true">
    <title key="radar-title">${label}</title>
    ...
    ${indicators.map((item, index) => {
      ...
      return html`<g key=${`radar-axis-${index}`}><line key=${`radar-axis-line-${index}`} x1=${cx} y1=${cy} x2=${edge.x} y2=${edge.y} stroke="#d4e4e0" stroke-width="1" vector-effect="non-scaling-stroke"/><circle key=${`radar-axis-point-${index}`} cx=${edge.x} cy=${edge.y} r="11" fill=${available ? '#eef8f7' : '#f1f5f4'} stroke=${available ? '#0e7490' : '#aab8b7'} stroke-width="1" vector-effect="non-scaling-stroke"><title>${`${item.title ?? item.name}: ${available ? `${fa(values[index])}٪` : 'ثبت نشده'}`}</title></circle><text key=${`radar-axis-label-${index}`} x=${edge.x} y=${edge.y + 4} text-anchor="middle" fill=${markerColor} font-size="12" font-weight="900" font-family="Vazirmatn">${fa(index + 1)}</text></g>`;
    })}
    ${hasData && segments.length === 1 && points.every(Boolean) && html`<polygon key="radar-area" points=${points.map(point => `${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(' ')} fill="rgba(14,116,144,.22)" stroke="none"/>`}
