...
function panelHeadingId(title) {
  const value = [...String(title)].reduce((total, character) => ((total * 31) + character.codePointAt(0)) >>> 0, 7);
  return `report-panel-${value.toString(36)}`;
}
...
function Panel({ title, tone = 'teal', className = '', children, action }) {
  ...
  return html`<section class=${`analytical-panel analytical-panel--${tone} ${className}`} aria-labelledby=${headingId}><header class="analytical-panel__header"><h3 id=${headingId}>${displayTitle}</h3>${action && html`<span>${action}</span>`}</header><div class="analytical-panel__body">${body}</div></section>`;
}
function Empty() { return html`<p class="analytical-empty">داده کافی نیست</p>`; }
function Stars({ value }) { const rounded = Math.round((Number(value) || 0) / 20); return html`<span class="report-stars" role="img" aria-label=${`${fa(value)} درصد`}>${[1, 2, 3, 4, 5].map(index => html`<span aria-hidden="true" class=${index <= rounded ? 'is-on' : ''}>★</span>`)}</span>`; }
...
function ReportCode({ code }) { return html`<span class="report-access-code" dir="ltr">${code}</span>`; }
...
export function AnalyticalReport({ snapshot, loading = false }) {
  ...
  return html`<article class="analytical-sheet" aria-label=${`کارنامه تحلیلی ${report.student.name}`}>
    ...
    <div class="analytical-sheet__grid">
      ...
      <${Panel} title="وضعیت آموزشی و نمرات نهایی" className="analytical-table" tone="teal"><table class="report-score-table"><caption class="sr-only">وضعیت آموزشی و نمرات نهایی ${report.student.name}</caption><thead><tr><th scope="col">درس / شاخص</th><th scope="col">مستمر</th><th scope="col">میان‌ترم</th><th scope="col">پایانی</th><th scope="col">میانگین</th><th scope="col">وضعیت</th></tr></thead><tbody>${report.subjects.slice(0, 8).map(subject => html`<tr><th scope="row">${subject.title}</th><td>${fa(subject.continuous)}</td><td>${fa(subject.midterm)}</td><td>${fa(subject.final)}</td><td class=${subject.current < 12 ? 'is-alert' : 'is-current'}>${fa(subject.current)}</td><td>${subject.passed === false ? html`<b class="is-alert">پیگیری</b>` : html`<b class="is-ok">قبول</b>`}</td></tr>`)}</tbody></table>${!report.subjects?.length && html`<${Empty}/>`}</${Panel}>
      ...
      <${Panel} title="کد پیگیری کارنامه" className="analytical-access" tone="navy"><div class="report-access"><${ReportCode} code=${report.accessCode}/><strong>مشاهده پس از ورود ایمن</strong><small>برای مشاهدهٔ نسخهٔ کامل، وارد سامانه شوید.</small></div></${Panel}>
    </div>
