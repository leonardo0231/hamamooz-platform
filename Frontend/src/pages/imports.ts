...
interface ImportError { row: number | null; message: string; }
...
async function showErrors(job: ImportJob): Promise<void> {
  ...
  const table = h('table', { className: 'data-table' },
    ...
    h('thead', {}, h('tr', {}, h('th', { scope: 'col', text: 'ردیف' }), h('th', { scope: 'col', text: 'شرح خطا' }))),
    h('tbody', {}, ...errors.map(item => h('tr', {}, h('td', { dataset: { label: 'ردیف' }, text: item.row ? formatNumber(item.row) : '—' }), h('td', { dataset: { label: 'شرح خطا' }, text: item.message })))),
  );
...
async function openImportDialog(onCreated: () => Promise<void>): Promise<void> {
  ...
  typeInput.addEventListener('change', () => void updatePreview());
  ...
  const form = h('form', { method: 'dialog', className: 'dialog__body' },
    ...
    h('label', { className: 'form-field' }, h('span', { text: 'فایل (.xlsx یا .xls)' }), fileInput),
    preview, progress,
...
export async function renderImportsPage(): Promise<HTMLElement> {
  ...
  page.append(h('div', { className: 'page-heading' }, h('div', {}, h('span', { className: 'eyebrow', text: 'ورود گروهی' }), h('h1', { text: 'ورود اطلاعات از فایل' }), h('p', { text: 'پیش‌نمایش و کنترل ساختار فایل پیش از ارسال انجام می‌شود؛ اعتبارسنجی نهایی در Backend است.' })), h('button', { className: 'button button--primary', type: 'button', disabled: !writeScopeReady, title: writeScopeReady ? 'بارگذاری فایل' : 'ابتدا حوزه فعال را انتخاب کنید', onClick: () => void openImportDialog(load) }, icon('upload'), 'بارگذاری فایل')),
    h('div', { className: 'import-guide-grid' }, ...Object.entries(importTypeLabels).map(([type, label]) => h('article', { className: 'card guide-card' }, icon(type === 'students' ? 'users' : type === 'scores' ? 'chart' : type === 'monthly_evaluations' ? 'check' : 'building'), h('h2', { text: label }), h('p', { text: `Template رسمی ${label} را دریافت و سپس فایل را بارگذاری کنید.` }), h('button', { className: 'button button--secondary', type: 'button', onClick: () => void apiRequest<Blob>(endpoints.imports.template(type), { responseType: 'blob' }).then(blob => downloadBlob(blob, `${type}_template.xlsx`)).catch(error => toast('دریافت Template ناموفق بود', 'error', error instanceof Error ? error.message : undefined)) }, icon('download'), 'دانلود Template')))),
    list,
  );
