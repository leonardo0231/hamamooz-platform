...
export function ReportsPage() {
  ...
  const openPreview = async item => {
    ...
    try {
      const archive = await apiRequest(`reports/${item.report_id}/`);
  ...
  return html`<div class="page report-workspace">
    ...
    <section class="report-preview-zone">
      <header class="report-preview-zone__header"><div><span>پیش‌نمایش تعاملی</span><h2>${previewSnapshot ? 'کارنامهٔ انتخاب‌شده' : 'نمونهٔ کارنامهٔ قابل ارائه'}</h2><p>${previewSnapshot ? 'این نمایش از snapshot نهاییِ آرشیوشده ساخته شده است.' : 'همان ساختار در خروجی PDF استاندارد A3 و A4 استفاده می‌شود.'}</p></div><div class="report-preview-zone__legend"><i></i>داده‌های آموزشی، رفتاری، حضور و فعالیت‌ها</div></header>
    ...
    <${Card} className="report-create" title="تولید گروهی کارنامه" subtitle="انتخاب‌ها فقط در محدودهٔ دسترسی فعلی شما اعتبارسنجی می‌شوند. برای خروجی رسمی، نمرات همهٔ دروس باید نهایی و قفل شده باشند.">
      <form onSubmit=${create} class="report-create__form">
        ...
        <label>اندازهٔ PDF<select value=${form.page_size} onInput=${event => updateForm('page_size', event.currentTarget.value)}><option value="a3_landscape">A3 افقی · یک صفحهٔ جامع</option><option value="a4_portrait">A4 عمودی · چندصفحهٔ خوانا</option></select></label>
    ...
    <${Card} title="بسته‌های تولیدشده" subtitle="خطای یک دانش‌آموز باعث توقف دیگر خروجی‌ها نمی‌شود. با انتخاب هر موردِ آماده، همان کارنامهٔ نهایی در بالا نمایش داده می‌شود." action=${html`<${Button} variant="outline" onClick=${load}>به‌روزرسانی</${Button}>`}>
      ${loading ? html`<${Skeleton} lines=${6}/>` : error ? html`<${ErrorState} error=${error} onRetry=${load}/>` : html`<div class="report-batches">${batches.map(batch => html`<article class="report-batch"><header><div><strong>${batch.scope === 'school' ? 'کل مدرسه' : 'کلاس منتخب'}</strong><small>${batch.page_size === 'a3_landscape' ? 'A3 افقی · داشبورد جامع' : 'A4 عمودی · نسخهٔ خوانا'}</small></div><${Badge} tone=${statusTone(batch.status)}>${statusLabel(batch.status)}</${Badge}></header><${Progress} value=${batch.progress_percent} label="پیشرفت تولید"/><footer><span>${fa(batch.completed_count)} آماده از ${fa(batch.total_count)} · ${fa(batch.failed_count)} ناموفق</span>${batch.zip_download_url && html`<a class="button button--outline" href=${batch.zip_download_url}>دانلود ZIP</a>`}</footer>${batch.items?.length ? html`<div class="report-batch__students">${batch.items.map(item => html`<div><span><b>${item.student_name}</b><small>${item.national_id}</small></span><${Badge} tone=${statusTone(item.status === 'completed' ? 'completed' : item.status === 'failed' ? 'failed' : 'processing')}>${statusLabel(item.status)}</${Badge}>${item.report_id && html`<button class="report-batch__preview" onClick=${() => void openPreview(item)}>نمایش</button>`}</div>`)}</div>` : null}</article>`)}${!batches.length && html`<p class="report-empty">هنوز بسته‌ای تولید نشده است.</p>`}</div>`}
    </${Card}>
  </div>`;
}
