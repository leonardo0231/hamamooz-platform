...
| R7-CI-01 | Make backend Ruff and integration CI green; Docker validation alone is not sufficient evidence. | **Open — latest head CI pending** | [Backend CI run 34244283466](https://github.com/leonardo0231/hamamooz-platform/actions/runs/34244283466) for `8c597e67` is in progress; the immediately preceding run for `a4170a5f` failed. No green post-preview-handoff backend run is available yet. | Wait for the final-head workflow, fix any Ruff/integration failures, rerun the complete backend checks, and attach a green run. |
| C9-ALL-OUTPUTS | Ensure every user-facing report preview/output is handed off to the React report bundle. | **Closed in current code; regression evidence retained** | [`67afb78`](https://github.com/leonardo0231/hamamooz-platform/commit/67afb78a0b3663cb112af67c3e45d5fe4e225e11) changes `ReportArchive.preview` to return an authorized snapshot plus a React print URL/renderer contract; [`692eed9`](https://github.com/leonardo0231/hamamooz-platform/commit/692eed9b48f65adf648ac655e7c2630f98dce0f6) formats that hand-off. The API contract test rejects the legacy Django report markup. | Keep the React hand-off contract covered in CI; this closure does not waive the real Chromium/A3 artifact or school-input gates. |
...
| Cycle-9 strict VP review · 2026-09-08 · remote head [`26365de`](https://github.com/leonardo0231/hamamooz-platform/commit/26365decde0547600f00ad8fb27fe9b77ed9bdfe) | **CONDITIONAL — INPUTS/INFRASTRUCTURE REQUIRED (NOT APPROVED)** | The preview hand-off from [`67afb78`](https://github.com/leonardo0231/hamamooz-platform/commit/67afb78a0b3663cb112af67c3e45d5fe4e225e11) through [`26365de`](https://github.com/leonardo0231/hamamooz-platform/commit/26365decde0547600f00ad8fb27fe9b77ed9bdfe) closes `C9-ALL-OUTPUTS`: `/reports/preview/` returns an authorized snapshot and React/A3 print contract, not the legacy Django report markup. The latest backend workflow [34244283466](https://github.com/leonardo0231/hamamooz-platform/actions/runs/34244283466) for the preceding renderer head is not green/complete, and no green post-preview-handoff backend run is available; `R7-CI-01` therefore remains open. Approval remains blocked by `C9-A3/C7-04` (no real Chromium-generated one-page A3 artifact and 100% visual verification), `C7-05` (authorized student photo and school-logo association), and `C7-06` (authoritative grades, scale, signer names/signatures/seal). Active production rendering remains Chromium-only with no WeasyPrint fallback. |
