...
| High | Current isolated development runtime does not provide Django, DRF, pytest, Ruff, WeasyPrint, project dependency cache, PostgreSQL or Docker | Runtime import probes and unavailable Docker executable during report-card implementation | Migrations, generated OpenAPI contract, backend pytest and actual PDF/DOCX visual output cannot be honestly verified in this environment | Run the documented migration/schema/pytest/PDF checks in the supported Compose stack or a provisioned Python 3.12 environment before deployment |
